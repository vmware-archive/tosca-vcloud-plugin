...
def net_connect_to_nat(vca_client, **kwargs):
    if ctx.target.node.properties['use_external_resource'] is True:
...
def prepare_network_operation(vca_client, operation):
    try:
        network_name = \
            ctx.source.instance.runtime_properties[VCLOUD_NETWORK_NAME]
...
def prepare_vm_operation(vca_client, operation):
    try:
        ...
        original_port = \
            ctx.target.node.properties['rules'].get('original_port', "any")
        translated_port = \
            ctx.target.node.properties['rules'].get('translated_port', "any")
    except KeyError as e:
        raise cfy_exc.NonRecoverableError("Parameter not found: {0}".format(e))
    ...
    for rule in rule_type:
        for ip in private_ip:
            ctx.logger.info(
                ...
                "translated_port {5}'".format(ip, public_ip, rule, protocol,
            ...
            if rule == "DNAT":
                function(rule, public_ip, str(original_port), ip,
...
def _get_network_ip_range(vca_client, org_name, network_name):
    ...
    for scope in ip_scope:
        for ip in scope[0].IpRanges.IpRange:
            scopes.append("{0} - {1}".format(ip.get_StartAddress(),
                                             ip.get_EndAddress()))
    return scopes
...
def _get_gateway_ip_range(gateway, network_name):
    ...
    for pool in gateway.get_dhcp_pools():
        if pool.Network.name == network_name:
            scopes.append("{0} - {1}".format(pool.get_LowIpAddress(),
